require "role_requirement_system/version"
    ...
    def user_authorized_for?(user, params = {}, binding = self.binding)
      ...
      self.role_requirements.each{| role_requirement|
        ...
        roles.each { |role|
          ...
          logger.debug role
          logger.debug user.has_role?(role)
          logger.debug "===end has_role==="
  ...
  module RoleSecurityInstanceMethods
    ...
    def access_denied
      if logged_in?
        ...
        redirect_to "/dashboard/access_denied"
        #return false
      else
        super
      end
    ...
    def check_roles
      logger.debug "====check_roles======"
      logger.debug current_user.inspect
      logger.debug params
      logger.debug binding.inspect
      logger.debug self.class.user_authorized_for?(current_user, params, binding)
      logger.debug "===end check_roles==="       
